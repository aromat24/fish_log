...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        setupGameSystem(); // New fishing game functionality
...
function setupTabSystem() {
    ...
    function switchTab(activeTab, activeContent) {
        ...
        if (leftGroup && rightGroup) {
            ...
            if (activeTab === historyTab) {
                ...
                // Add Records, Map, and Game to right group in sequence
                ...
                // Game active: all other tabs slide left, Game gets label
                gameTab.classList.add('active');
                leftGroup.appendChild(recordsTab);
                leftGroup.appendChild(mapTab);
                leftGroup.appendChild(gameTab);
                labelElement.textContent = gameTab.getAttribute('data-label');
                leftGroup.appendChild(labelElement);
            }
        }
    ...
    // Game tab click handler with toggle behavior
    if (gameTab) {
        gameTab.addEventListener('click', () => {
            console.log('Game tab clicked');
            // If game tab is already active, switch to history
            if (gameTab.classList.contains('active')) {
                switchTab(historyTab, catchLog);
            } else {
                switchTab(gameTab, gameContainer);
                
                // Initialize or refresh game when tab is activated
                setTimeout(() => {
...
// Fishing Game System Setup
function setupGameSystem() {
    console.log('=== SETTING UP GAME SYSTEM ===');
    ...
    try {
        ...
        if (gameHelpBtn) {
            ...
            console.log('✅ Game help button setup');
        }
        ...
        if (gameSettingsBtn) {
            ...
            console.log('✅ Game settings button setup');
        }
        
        console.log('✅ Game system setup complete');
        
    } catch (error) {
        console.error('❌ Game system setup failed:', error);
        if (window.errorHandler) {
            window.errorHandler.logError(
                new Error('Game system setup failed'),
                'setupGameSystem',
                { originalError: error }
            );
        }
...
async function initializeFishingGame() {
    ...
    try {
        // Update game status
        updateGameStatus('Initializing game engine...');
        
        // Check if required classes are available
        if (!window.FishingGameEngine) {
            throw new Error('FishingGameEngine class not available');
        }
        
        if (!window.FishingGameMenuScene) {
            throw new Error('FishingGameMenuScene class not available');
        }
        
        // Create game engine if not exists
        if (!fishingGameEngine) {
            fishingGameEngine = new window.FishingGameEngine('fishing-game-canvas', {
        ...
        const engineReady = await fishingGameEngine.init();
        if (!engineReady) {
        ...
        fishingGameEngine.addScene('menu', menuScene);
        
        // Set initial scene
        fishingGameEngine.setScene('menu');
        
        // Start the game loop
        fishingGameEngine.start();
        
        // Update status and FPS display
        updateGameStatus('Game ready - Touch to play!');
        startFPSMonitor();
        
        console.log('✅ Fishing game initialized successfully');
        
    } catch (error) {
        console.error('❌ Fishing game initialization failed:', error);
        updateGameStatus('Game initialization failed');
        
        if (window.errorHandler) {
            window.errorHandler.logError(
                new Error('Fishing game initialization failed'),
                'initializeFishingGame',
                { originalError: error }
            );
            
            window.errorHandler.showUserError(
                'The fishing game could not be loaded. Please refresh the page and try again.'
            );
        }
    }
}
...
function updateGameStatus(message) {
    try {
        ...
        console.warn('Failed to update game status:', error);
    }
}
...
function startFPSMonitor() {
    try {
        ...
        if (!fpsElement || !fishingGameEngine) return;
        
        setInterval(() => {
            if (fishingGameEngine && fishingGameEngine.isHealthy()) {
                const stats = fishingGameEngine.getPerformanceStats();
                const fps = fishingGameEngine.fps || 0;
                const health = fishingGameEngine.isHealthy() ? '✅' : '⚠️';
                fpsElement.textContent = `${health} ${fps} FPS`;
                
                // Color code based on performance
                if (fps >= 50) {
                    fpsElement.style.color = '#16a34a'; // Green
                } else if (fps >= 30) {
                    fpsElement.style.color = '#eab308'; // Yellow
                } else {
                    fpsElement.style.color = '#ef4444'; // Red
                }
            } else {
                fpsElement.textContent = '❌ Engine Error';
        ...
        console.warn('FPS monitor setup failed:', error);
    }
}
...
🎯 How to Play:
• Touch and drag to cast your line
• Wait for fish to bite (watch for ripples)
• Tap quickly when you see the bite indicator
• Fight the fish by following on-screen prompts

🐟 Fish Species:
• Different fish prefer different depths
• Larger fish are harder to catch
• Learn about real fish from your logbook data

🏆 Tips:
• Start with Quick Fish for easy gameplay
• Unlock new locations by catching fish
• Check the Fish Guide to learn species info

Touch anywhere to dismiss this help.
        `;
        
        updateGameStatus('Showing help...');
        
        // Show simple alert for now - could be enhanced with a modal
        alert(helpMessage.trim());
        
        updateGameStatus('Ready to Fish!');
        
    } catch (error) {
        console.error('Show game help failed:', error);
    }
}
...
🔧 Current Settings:
• Debug Mode: ${fishingGameEngine?.config?.debugMode ? 'On' : 'Off'}
• Error Recovery: ${fishingGameEngine?.config?.enableErrorRecovery ? 'On' : 'Off'}
• Target FPS: ${fishingGameEngine?.config?.targetFPS || 60}

📊 Performance:
• Current FPS: ${fishingGameEngine?.fps || 'Unknown'}
• Errors: ${fishingGameEngine?.errorCount || 0}
• Engine Health: ${fishingGameEngine?.isHealthy() ? 'Good' : 'Poor'}

Note: Advanced settings will be added in future updates.

Touch anywhere to dismiss.
        `;
        
        updateGameStatus('Showing settings...');
        
        // Show simple alert for now - could be enhanced with a modal
        alert(settingsMessage.trim());
        
        updateGameStatus('Ready to Fish!');
        
    } catch (error) {
        console.error('Show game settings failed:', error);
    }
}
